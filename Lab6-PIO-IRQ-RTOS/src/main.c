...
void but_callback(void) {
  int delay; 
  delay -= 100;
  xQueueSendFromISR(xQueueButValue, &delay, 10);  
}
...
static void task_but(void *pvParameters) {
  ...
  for (;;) {
	...
	//if (xSemaphoreTake(xSemaphoreBut, 1000))
     if (xQueueReceive(xQueueButValue, &delay, (TickType_t)0)){
      delayTicks += delay;
...
int main(void) {
  ...
  if (xSemaphoreBut == NULL)
	...
	printf("falha em criar o semaforo do but1 \n");
  ...
  if (xQueueButValue == NULL)
	printf("falha em criar a queue dos botões\n");
